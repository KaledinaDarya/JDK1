...
import static by.http.web.utils.WebConstant.REQ_PARAM_RECIPE_TIME;
...
public class EditRecipeTimeActionImpl implements BasicAction {
	...
	public void perfomAction(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		long recipeTime = Integer.parseInt(req.getParameter(REQ_PARAM_RECIPE_TIME));
		
		if (recipeId >= 0) {
			System.out.println("recipeId: " + recipeId);
			if (catalogService.findRecipeFromCatalogById(recipeId) != null) {
				catalogService.editRecipeTimeFromCatalog(recipeId, recipeTime);	
			} else {
