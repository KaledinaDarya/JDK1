...
import static by.http.web.utils.WebConstant.REQ_PARAM_RECIPE_FLAG_VISIBLE;
...
public class EditRecipeFlagVisibleActonImpl implements BasicAction {
	...
	public void perfomAction(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		boolean recipeFlagVisible = Boolean.parseBoolean(req.getParameter(REQ_PARAM_RECIPE_FLAG_VISIBLE));
		
		if (recipeId >= 0) {
			System.out.println("recipeId: " + recipeId);
			if (catalogService.findRecipeFromCatalogById(recipeId) != null) {
				catalogService.editRecipeFlagVisibleFromCatalog(recipeId, recipeFlagVisible);	
			} else {
